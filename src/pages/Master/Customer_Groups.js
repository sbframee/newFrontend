...
const Customer_Groups = () => {
  ...
  const getItemsData = async () => {
    const response = await axios({
      method: "get",
      url: "http://localhost:9000/groups/GetCustomer_GroupList",
...
export default Customer_Groups;
    ...
    >
      ...
      {itemsDetails?.map((item, i) => (
            ...
            >
              ...
              <td colSpan={3}>{item?.customer_group}</td>
    ...
    try {
      ...
      if (popupInfo?.type === 'edit') {
        response = await axios.put(
          'http://localhost:9000/groups/putCustomer_Groups',
          [formData],
          {
            headers: {
              'Content-Type': 'application/json',
        ...
        response = await axios.post(
          'http://localhost:9000/groups/postCustomer_group',
          formData,
          {
            headers: {
              'Content-Type': 'application/json',
